...
#[superstruct(
    variants(V17, BlockV29, EnvelopeV29),
...
pub struct ProtoNode {
    ...
    #[superstruct(only(BlockV29))]
    pub payload_invalid: bool,
...
pub struct ProtoArray {
    ...
    pub envelope_indices: HashMap<(Hash256, bool), usize>,
}
...
impl ProtoArray {
    ...
    pub fn apply_score_changes<E: EthSpec>(
        ...
        for node_index in (0..self.nodes.len()).rev() {
            ...
            match node {
                ...
                ProtoNode::BlockV29(_) => {
                    // FIXME(gloas): may be illegal state?

                }
                ProtoNode::EnvelopeV29(node) => {
        ...
        for node_index in (0..self.nodes.len()).rev() {
            ...
            match node {
                ...
                ProtoNode::BlockV29(_) => {
                    //FIXME(gloas): this is maybe illegal weight update
                }
                ProtoNode::EnvelopeV29(node) => {
                    if let Some(parent_index) = node.parent {
                        self.maybe_update_best_child_and_descendant::<E>(
    ...
    pub fn on_block<E: EthSpec>(
        ...
        let node = if chain_spec
            .fork_name_at_slot::<E>(current_slot)
            .gloas_enabled()
        {
            ...
            // This can potentially be implemented better but currently here
            // we are trying to get the block parent from index and building the next block
            // on top of the best envelope (either PayloadStatus::Empty or PayloadStatus::Full)
            //
            // we don't currently don't check whether a give fork is valid to build upon.
            // by valid fork is not a chain of block with valid payloads.
            let parent_index = block
                    .parent_root
                    .and_then(|parent_root|
                        {
                        let parent_block_index = self.indices.get(&parent_root).copied()?;
                        let parent_block = self.nodes.get(parent_block_index)?;
                        *parent_block.best_descendant()
                    });

            ProtoNode::BlockV29(ProtoNodeBlockV29{
            slot: block.slot,
            root: block.root,
            target_root: block.target_root,
            current_epoch_shuffling_id: block.current_epoch_shuffling_id,
        ...
        match node {
            ProtoNode::V17(node) => {
                if let Some(parent_index) = node.parent {
                    ...
                    match parent {
                        ProtoNode::V17(parent_node) => {
                            if parent_node.execution_status.is_invalid() {
                        ...
                        _ => return Err(Error::UnexpectedNodeVariant)
                    }
    ...
    pub fn find_head<E: EthSpec>(
        ...
        match justified_node {
            ProtoNode::V17(justified_node) => {
                ...
                match best_node {
                    ProtoNode::V17(best_node) =>{

                        // Perform a sanity check that the node is indeed valid to be the head.
                        if !self.node_is_viable_for_head::<E>(
                            &ProtoNode::V17(best_node.clone()),
                            current_slot,
                            best_justified_checkpoint,
                            best_finalized_checkpoint,
                        ) {
                    ...
                    ProtoNode::BlockV29(_) =>{ Err(Error::GloasNotImplemented)}
                    ProtoNode::EnvelopeV29(_) =>{ Err(Error::GloasNotImplemented)}
                }
            }
    ...
    pub fn maybe_prune(&mut self, finalized_root: Hash256) -> Result<(), Error> {
        ...
        for node_index in 0..finalized_index {
            let node = self
                ...
                .ok_or(Error::InvalidNodeIndex(node_index))?;
            let root = match node {
                ProtoNode::V17(inner) => inner.root,
                ProtoNode::BlockV29(inner) => inner.root,
                ProtoNode::EnvelopeV29(inner) => inner.root,
            };
            ...
            match node {
                ProtoNode::V17(inner) => {
                    if let Some(parent) = inner.parent {
                        inner.parent = parent.checked_sub(finalized_index);
                    }
                    if let Some(best_child) = inner.best_child {
                        inner.best_child = Some(
                            best_child
                                .checked_sub(finalized_index)
                                .ok_or(Error::IndexOverflow("best_child"))?,
                        );
                    }
                    if let Some(best_descendant) = inner.best_descendant {
                        inner.best_descendant = Some(
                            best_descendant
                                .checked_sub(finalized_index)
    ...
    fn node_is_viable_for_head<E: EthSpec>(
        ...
        match node {
            ProtoNode::V17(inner) => {
                if inner.execution_status.is_invalid() {
                    return false;
                }

                let node_epoch = inner.slot.epoch(E::slots_per_epoch());

                let voting_source = if current_epoch > node_epoch {
                    // The block is from a prior epoch, the voting source will be pulled-up.
                    inner
                        .unrealized_justified_checkpoint
                        .unwrap_or(inner.justified_checkpoint)
                } else {
                    // The block is not from a prior epoch, therefore the voting source
                    // is not pulled up.
                    inner.justified_checkpoint
                };
            ...
            ProtoNode::EnvelopeV29(_) => {
                // Envelopes are not viable for head
                false
            }
        }
    }
    ...
    pub fn is_finalized_checkpoint_or_descendant<E: EthSpec>(
        ...
        loop {
            match node {
                ProtoNode::V17(inner) => {
                    // Check finalized/justified checkpoints
                    if inner.finalized_checkpoint == best_finalized_checkpoint
                        || inner.justified_checkpoint == best_finalized_checkpoint
                    {
                        return true;
                    }

                    // Check unrealized checkpoints
                    if inner.unrealized_finalized_checkpoint == Some(best_finalized_checkpoint)
                        || inner.unrealized_justified_checkpoint == Some(best_finalized_checkpoint)
                    {
                        return true;
                    }

                    // If node is <= finalized slot, it must be the finalized block
                    if inner.slot <= finalized_slot {
                        return inner.root == finalized_root;
                    }

                    // Walk up to parent
                    if let Some(parent) = inner.parent.and_then(|index| self.nodes.get(index)) {
                        node = parent;
                    } else {
                        return false;
                    }
                ...
                ProtoNode::EnvelopeV29(inner) => {
                    // Envelopes don't have checkpoints, just walk to parent
                    if inner.slot <= finalized_slot {
                        return false; // Envelopes can't be finalized checkpoint
                    }
    ...
    pub fn execution_block_hash_to_beacon_block_root(
        ...
        self.nodes.iter().rev().find_map(|node| match node {
            ...
            // BlockV29 and EnvelopeV29 don't have execution_status
            ProtoNode::BlockV29(_) | ProtoNode::EnvelopeV29(_) => None,
        })
    }
